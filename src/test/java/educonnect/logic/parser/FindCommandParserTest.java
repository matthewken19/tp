...
public class FindCommandParserTest {
    ...
    public void parse_multipleArg_success() {
        ...
        FindCommand expectedCommand = new FindCommand(List.of(VALID_NAME_PREDICATE, VALID_TAG_PREDICATE));
        assertParseSuccess(parser, userInput, expectedCommand);

        userInput = STUDENT_ID_DESC_AMY + NAME_DESC_AMY;
        expectedCommand = new FindCommand(List.of(VALID_STUDENT_ID_PREDICATE, VALID_NAME_PREDICATE));
        assertParseSuccess(parser, userInput, expectedCommand);

        userInput = TELEGRAM_HANDLE_DESC_AMY + TAG_DESC_FRIEND;
        expectedCommand = new FindCommand(List.of(VALID_TELEGRAM_HANDLE_PREDICATE, VALID_TAG_PREDICATE));
        assertParseSuccess(parser, userInput, expectedCommand);

        userInput = EMAIL_DESC_AMY + TELEGRAM_HANDLE_DESC_AMY;
        expectedCommand = new FindCommand(List.of(VALID_EMAIL_PREDICATE, VALID_TELEGRAM_HANDLE_PREDICATE));
        assertParseSuccess(parser, userInput, expectedCommand);
    }
}
